...
清理媒体库中的专辑/曲目文件（**同步**）。粒度：`tracks` 指定曲目 > `album` 整专辑 > `artist` 整艺人。`tracks` 可单独配合 `artist`（不传 `album`）：此时在艺人目录内直接匹配曲目，适配 singles 等无专辑层级的库；匹配不到返回 400，不动任何文件。删除曲目后**空目录自底向上一并清理**（不留空目录）：空 `CDx/` → 无音频残留的专辑目录（连同 `cover.jpg`/`album_info.txt`/`lyrics/`）→ 空艺人目录。删除文件时同名 `.lrc`（`lyrics/` 内与文件旁的）一并删除。

> **高危操作确认**：整艺人删除（`album`/`tracks` 均留空）与整专辑删除（仅 `album`）会 `rmtree` 整个目录、不可恢复，非 `dry_run` 时必须传 `confirm: true`，否则返回 400；曲目级删除与 `dry_run` 不需要 `confirm`。
...
| `library` | 否 | 默认库 | 库名（见 `/api/v1/libraries`） |
| `artist` | 是 | — | 艺人名（对应库内一级目录） |
| `album` | 否 | — | 专辑名（对应库内二级目录）；与 `tracks` 同时留空则清理整个艺人目录（需 `confirm`） |
| `tracks` | 否 | — | 要清理的曲目：序号（如 `3`）、`"D-NN"`（多碟消歧，同 replace_track）或曲名（相似度 ≥0.7 取最高分）；不传 `album` 时在艺人目录内匹配；留空则清理整个专辑（需 `confirm`） |
| `dry_run` | 否 | false | 只报告将删除的项，不实际删除（**建议先跑一遍确认范围**；无需 `confirm`） |
| `confirm` | 否 | false | 整艺人/整专辑删除需显式传 `true` 确认；曲目级删除不需要 |
...
**400**：未知库名 / 艺人或专辑目录不存在 / 曲目未命中 / 整目录删除未传 `confirm` / 目录名解析后越出库根（如 `..`；`album="."` 不拦截，按整艺人目录处理）
